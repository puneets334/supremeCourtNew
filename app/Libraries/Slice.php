...
class Slice
{
	...
	public function getSegment($segmentNumber) {
        log_message('debug', 'Fetching URL segment: ' . $segmentNumber);
        return $this->uri->getSegment($segmentNumber);
    }

	public function renderOutput() {
        // Check if the output property is initialized
        if (is_null($this->output)) {
            log_message('error', 'Output service is not initialized.');
            throw new \RuntimeException('Output service is not initialized.');
        }
        return $this->output->getBody();  // Example method call, adjust as necessary
    }
