...
class DefaultController extends BaseController {
    ...
    public function index() {
        ...
        //echo '<pre>';print_r($this->session->userdata['login']);echo '</pre>';exit();
        $this->slice->view('responsive_variant.adjournment_letter.index',compact('scheduled_cases','existing_requests','existing_requests_others'));
    }
    public function doAdjournmentRequest($data){
        $this->check_login();
        $request_data=url_decryption($data);
        $parameters=explode('#',$request_data);
        $data_array=array('diary_number'=>$parameters[0],
        ...
        $this->slice->view('responsive_variant.adjournment_letter.move_adjournment',compact('data_array'));
    }
