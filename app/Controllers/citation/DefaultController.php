...
class DefaultController extends BaseController {
    ...
    public function index($id = NULL) {
        ...
        if ($id) {
            ...
            if (!is_numeric($InputArrray[0]) || $InputArrray[1] != E_FILING_TYPE_CITATION || !is_numeric($InputArrray[2])) {
                redirect('login');
                exit(0);
            }
            ...
            $estab_details = $this->Citation_model->get_establishment_details();

            if ($estab_details) {

                //SET $_SESSION['efiling_details']
                $efiling_num_details = $this->Citation_model->get_efiling_num_basic_Details($registration_id);
            ...
            $estab_details = $this->Citation_model->get_establishment_details();
            if ($estab_details) {
                $_SESSION['efiling_details']['ref_m_efiled_type_id'] = E_FILING_TYPE_CITATION;
           ...
           // if ($_SESSION['login']['ref_m_usertype_id'] == USER_ADMIN) {
            ...
            exit(0);*/
                switch (max(explode(',', $_SESSION['efiling_details']['breadcrumb_status']))) {
                    ...
                    case CITATION_BREAD_CITATION_DETAILS:
                        redirect('citation/CitationController');
