...
class Attendee extends CI_Controller {
    ...
    private function checkSeatAvailability($diary_no,$roster_id, $court_no){
        ...
        $seatsAllocatedToAor = $this->hearing_model->seatsAllocatedCount($roster_id,$diary_no, $this->session->loginData['bar_id'], $court_no);
    ...
    public function save(){
        ...
        elseif(@$visit_id!==false && !empty(@$visit_id) ){
            ...
            $checkCounselEntryForToday = $this->hearing_model->getCounselEntryForToday($_POST['case_info']['diary_no'], $_POST['case_info']['roster_id'], $_POST['mobile'], $_POST['case_info']['court_no']);
            ...
            if ($checkCounselEntryForToday[0]['counsel_entry'] < 1) {
                ...
                if ($checkDateAndTimeAllow == 1) {
                    $seatCheck = $this->checkSeatAvailability($_POST['case_info']['diary_no'], $_POST['case_info']['roster_id'], $_POST['case_info']['court_no']);
                    ...
                    if (($seatCheck == 1 && ($_POST['ref_attendee_type_id'] != 5 && $_POST['ref_attendee_type_id'] != 9)) || ($seatCheck==3 && $_POST['ref_attendee_type_id']==5) || ($seatCheck == 2 && $_POST['ref_attendee_type_id'] != 9)  ) {
                        ...
                        $attendee_data = array('diary_no' => $_POST['case_info']['diary_no'], 'roster_id' => $_POST['case_info']['roster_id'], 'next_dt' => $case_next_dt,  'case_number' => $_POST['case_info']['case_no'], 'item_no' => $_POST['case_info']['case_no'],  'ref_attendee_type_id' => $_POST['ref_attendee_type_id'], 'name' => $_POST['name'], 'email_id' => $_POST['email'], 'mobile' => $_POST['mobile'], 'display' => 'Y', 'created_by_advocate_id' => $this->session->loginData['bar_id'], 'ip_address' => get_client_ip(), 'court_no'=>$_POST['case_info']['court_no'],'item_no'=>$_POST['case_info']['item_no']);
    ...
    public function attendee_list($caseInfo){
        ...
        if($allowed_upto_timestamp>=$current_timestamp){
            $otherAttendee=$this->hearing_model->getAttendeeList($caseInfo['diary_no'],$caseInfo['roster_id'],$this->session->loginData['bar_id'],$caseInfo['court_no']);
            //Check for entry if not exists the make entry
            $this->doCaseAOREntry($caseInfo);
            $selfDetails=$this->hearing_model->getSelfEntry($caseInfo['diary_no'],$caseInfo['roster_id'],$this->session->loginData['bar_id'],$caseInfo['court_no']);
        }
    ...
    private function doCaseAOREntry($caseInfo){
        ...
        if(count($selfDetails)<=0){
            ...
            if(@$visit_id!==false && !empty(@$visit_id)) {
                ...
                $aor_self_data = array('diary_no' => $caseInfo['diary_no'], 'roster_id' => $caseInfo['roster_id'], 'next_dt' => $case_next_dt, 'case_number' => $caseInfo['case_no'], 'item_no' => $caseInfo['item_no'],  'ref_attendee_type_id' => 9, 'name' => $this->session->loginData['title'] . ' ' . $this->session->loginData['name'], 'email_id' => $this->session->loginData['email'], 'mobile' => $this->session->loginData['mobile'], 'display' => 'Y', 'created_by_advocate_id' => $this->session->loginData['bar_id'], 'ip_address' => get_client_ip(), 'court_no' => $caseInfo['court_no']);
    ...
    public function restore_attendee($id, $caseInfo){
        ...
        if($checkDateAndTimeAllow==1)
        {
            $totalAttendee=$this->hearing_model->getActiveAttedeeCount($caseInfo['diary_no'],$caseInfo['roster_id'],$this->session->loginData['bar_id'],$caseInfo['court_no']);
    ...
    private function getTomorrowRegularDailyListMainCases($next_date)
    {
        $schedule_request_params = array('forDate' => $next_date, 'fromDate' => $next_date, 'toDate' => $next_date, 'hearingType' => array('F'), 'responseFormat' => 'CASE_WISE_FLATTENED_WITH_ALL_INFO', 'ifSkipDigitizedCasesStageComputation' => true);
        $scheduled_cases = (array)json_decode(file_get_contents('https://10.0.0.1:4443/api/schedule/cases?'.http_build_query($schedule_request_params), false, stream_context_create(array('http' => array('user_agent' => 'Mozilla'), "ssl"=>array("verify_peer"=>false, "verify_peer_name"=>false)))));
        $tomorrow_daily_list_cases=array();
        foreach ($scheduled_cases[0] as $key => $value) {
            //echo $value->diary_id;
            if(empty($value->item_number_alt))
                array_push($tomorrow_daily_list_cases,$value->diary_id);
        }
        return $tomorrow_daily_list_cases;
    ...
    public function send_7am_reminder(){
       /* if(!$this->input->is_cli_request()){
        ...
        $today = date("Y-m-d");
        $advocates_id_to_which_mail_to_send=array();
        $next_date_regular_hearing_listed_case=$this->getTomorrowRegularDailyListMainCases($today);
        $weekly_list = $this->hearing_model->weekly_list_number();

        // we will send sms for all diary numbers-all advocates i.e we did not use court_no
        $list_number = $weekly_list[0]['weekly_no'];
        $list_year = $weekly_list[0]['weekly_year'];
        if(!empty($next_date_regular_hearing_listed_case)) { // sms send only if daily list of today is already published

            // get today listed cases all cases-advocate list
            $all_today_list_advocate_details = $this->hearing_model->get_list_all_advocate_of_today_daily_list_cases($next_date_regular_hearing_listed_case);
            $all_today_list_advocate_details=$all_today_list_advocate_details[0]['case_all_advocates'];
            $all_today_list_advocate_details= explode(",", $all_today_list_advocate_details);

            // get consent received till now for all cases-advocate list
            $advocate_consent_already_received_for_cases_list = $this->hearing_model->get_consent_received_from_advocate_all_cases($list_number,$list_year,$today,$next_date_regular_hearing_listed_case); // get consent recevied from advocate
            $advocate_consent_already_received_for_cases_list=$advocate_consent_already_received_for_cases_list[0]['consent_received_cases_advocates'];
            $advocate_consent_already_received_for_cases_list= explode(",", $advocate_consent_already_received_for_cases_list);

            $cases_list_in_which_advocate_consent_pending_till_now=array_diff($all_today_list_advocate_details,$advocate_consent_already_received_for_cases_list);
            if(!empty($cases_list_in_which_advocate_consent_pending_till_now))
            {
                foreach($cases_list_in_which_advocate_consent_pending_till_now as $case_advocate)
                {
                    $case_adovcate= array_map('intval', explode("_", $case_advocate));
                    array_push($advocates_id_to_which_mail_to_send,$case_adovcate[1]);
                }

                $advocate_details = $this->hearing_model->get_consent_not_received_cases_advocate_mobile_nos($advocates_id_to_which_mail_to_send);
...
public function printAttendeeFinalReport()
    {
        $atteldee_list_html=null;
        $today = date("Y-m-d");
        $next_date_regular_hearing_listed_case=$this->getTomorrowRegularDailyListCases($today);
        ...
        if(!empty($next_date_regular_hearing_listed_case)) {
            ...
            if (!empty($tentative_attendee_list)) {
                ...
                $atteldee_list_html=$this->load->view('email/attendee_list_mail',$data, true);
            }

            $this->load->library('M_pdf');
            $mpdf = new mPDF('utf-8', '', 14, 'Arial');
            $mpdf->autoScriptToLang = true;
            $mpdf->autoLangToFont = true;
            $stylesheet = file_get_contents('assets/css/myCSS.css');


            $mpdf = new mPDF('utf-8', 'A4-P', 11, 'Arial');
            $html = $atteldee_list_html;

            $water_marked_text = 'Physical Hearing (with Hybrid Option)';

            $mpdf->SetWatermarkText($water_marked_text);
            ...
            $pdf_content=$mpdf->Output("Final_Attendee_list_for_dated ".$today.'.pdf', 'S');
            echo $pdf_content;
        }
