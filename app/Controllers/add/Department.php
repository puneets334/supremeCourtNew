...
class Department extends BaseController {
    ...
    public function add_department() {
        ...
        if ($_SESSION['login']['ref_m_usertype_id'] == USER_SUPER_ADMIN) {
            if (empty($catag) || preg_match('/[^0-9a-zA-Z]/', $catag)) {
                $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>Please Select Valid Catagory.</div>");
        ...
        if (preg_match('/[^0-9]/i', $mobile) || strlen($mobile) != 10) {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>Please Enter Valid Mobile Number.</div>");

            redirect($redirection);
        }
        if (!preg_match($email_pattern, $email)) {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>Please Enter Valid Email ID.</div>");
            redirect($redirection);
        }
        ...
        if ($check_email_already_present) {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in' >Same Email Id already Registered.</div>");
            redirect($redirection);
        }
        if ($check_mobno_already_present) {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>Same Mobile Number already Registered. </div>");
            redirect($redirection);
        }
        if ($check_usrid_already_present) {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>Same User ID already Registered</div>");
            redirect($redirection);
        }

        if (preg_match('/[^0-9a-zA-Z. ]/', $department) || $department == NULL || strlen($department) < 5 || strlen($department) > 100) {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>Department Name is required and can contain only characters and spaces. Minimum length 5 and max length 100 Characters.</div>");
            redirect($redirection);
        }
        if (preg_match('/[^0-9a-zA-Z. ]/', $moniter) || $moniter == NULL || strlen($moniter) < 5 || strlen($moniter) > 100) {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>Moniter Name is required and can contain only characters and spaces.</div>");
            redirect($redirection);
        }

        if (preg_match('/[^0-9]/i', $ref_m_states_id[0])) {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>State is required and can have only numbers!</div>");
            redirect($redirection);
        }
        if (preg_match('/[^0-9]/i', $district_id[0])) {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>District is required and can have only numbers!</div>");
            redirect($redirection);
        }
        $validate_name = validate_names($address, TRUE, 4, 150, 'Address');
        if ($validate_name['response'] == FALSE) {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>" . htmlentities($validate_name['msg']['field_name'], ENT_QUOTES) . "</div>");
            redirect($redirection);
        }
        if (preg_match("/[^0-9a-zA-Z\s.:,-_\/ ]/i", $city) || $city == NULL) {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>City  is required and can contain only characters,numbers,spaces and :,.-_/</div>");
            redirect($redirection);
        }
        if (preg_match('/[^0-9a-zA-Z]/', $userid) || empty($userid) || strlen($userid) < 5 || strlen($userid) > 15) {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>Login id is required and can be in alphnumeric format with min. length 5 and max. length 15.</div>");
            redirect($redirection);
        }
        if ($_POST['pincode'] != '') {
            $pincode = $_POST['pincode'];
            if (preg_match('/[^0-9]/i', $_POST['pincode']) || (strlen($_POST['pincode']) != 6)) {
                $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'> Pincode is required and can have only numbers of 6 digit length </div>");
                redirect($redirection);
            }
        } else {
            $pincode = 0;
        ...
        if ($result) {
            $this->session->set_flashdata('message', "<div class='alert alert-success fade in'>Department Added Successfully.</div>");
            ...
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>Something Wrong, Please try again later.</div>");
            redirect($redirection);
        }
    ...
    public function update_department() {
        ...
        if (preg_match('/[^0-9a-zA-Z. ]/', $department) || $department == NULL || strlen($department) < 5 || strlen($department) > 100) {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>Department Name is required and can contain only characters and spaces.</div>");
            redirect($redirection);
        }
        if (preg_match('/[^0-9a-zA-Z. ]/', $moniter) || $moniter == NULL || strlen($moniter) < 5 || strlen($moniter) > 100) {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>Moniter Name is required and can contain only characters and spaces.</div>");
        ...
        if ($result) {
            $this->session->set_flashdata('message', "<div class='alert alert-success fade in'>Department Updated Successfully.</div>");
            redirect($redirection);
        } else {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>Something Wrong, Please try again later.</div>");
            redirect($redirection);
    ...
    public function department_advocate_panel() {
        ...
        $this->load->view('templates/header');
        $this->load->view('department/advocate_panel_view', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function search_advocate() {
        ...
        if (!empty($adv_mobile)) {
            if (preg_match('/[^0-9]/i', $adv_mobile) || strlen($adv_mobile) != 10) {
                $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>Please Enter Valid Mobile Number.</div>");
                redirect('add/Department/department_advocate_panel');
            }
        }
        $adv_barcode = trim($_POST['bar_code']);
        if (empty($first_name) && empty($adv_mobile) && empty($adv_barcode)) {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>Please Enter Advocate Name or Mobile or Barcode.</div>");
            redirect('add/Department/department_advocate_panel');
        }
        ...
        if (empty($data['advocate_data'])) {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>No Record Found or Not register in eFiling.</div>");
            redirect('add/DepDartment/department_advocate_panel');
        }
    ...
    public function add_dept_advocate() {
        ...
        for ($i = 0; $i < $input_count; $i++) {
            ...
            if ($advocate_id == $get_data[$i]['advocate_id'] || !empty($get_data)) {
                $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>Already Exist.</div>");
        ...
        if ($result) {
            $this->session->set_flashdata('message', "<div class='alert alert-success fade in'>Added Successfully.</div>");
            redirect('add/department/department_advocate_panel');
        } else {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>Something went wrong! Please try again later.</div>");
        }
    ...
    function delete_dept_advocate($advid, $depid) {
        ...
        if ($result) {
            $this->session->set_flashdata('message', "<div class='alert alert-success fade in'>Deleted Successfully.</div>");
            redirect('add/Department/department_advocate_panel');
        } else {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>Something went wrong! Please try again later.</div>");
            redirect('add/Department/department_advocate_panel');
