...
class AutoDiary extends BaseController
{
        ...
        if ($result) {
            ...
            if (in_array(getSessionData('efiling_details')['stage_id'], $marked_defect_tobe_shown_stages)) {
                ...
                if (!empty($isSuccessfullyAproved)) {
                    $autoGeneratedNo = $this->generateAutoDiary($registration_id); //function written for Auto diarization in the last week
                }
            }
        } else {
            echo "error";
    ...
    public function generateAutoDiary($registration_id = null)
    {
        if (empty($registration_id)) {
            ...
            // redirect('newcase/AutoDiaryGeneration/byAOR');exit();

            $curl = curl_init();
            curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($curl, CURLOPT_URL, ADMIN_SERVER_URL . 'newcase/AutoDiaryGeneration');
