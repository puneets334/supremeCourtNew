...
class Extra_party extends CI_Controller {
    ...
    public function index($party_id = NULL) {
        ...
        if (isset($_SESSION['efiling_details']['registration_id']) && !empty($_SESSION['efiling_details']['registration_id'])) {
            ...
            if (isset($party_id) && !empty($party_id)) {
                ...
                $data['party_details'] = $this->Get_details_model->get_case_parties_details($registration_id, array('p_r_type' => NULL, 'm_a_type' => NULL, 'party_id' => $party_id, 'view_lr_list'=> FALSE));

                $data['district_list'] = $this->Dropdown_list_model->get_districts_list($data['party_details'][0]['state_id']);
            }
            //echo "<pre>"; print_r($data['party_details']); die;
        }
    ...
    public function add_extra_party($party_id = NULL) {
        ...
        $this->form_validation->set_rules('party_as', 'Party As', 'required|in_list[I,D1,D2,D3]');
        $this->form_validation->set_rules('p_r_type', 'Extra Party Side', 'required|in_list[P,R]');
        if ($_POST['party_as'] == 'I') {
            ...
            /*START OF CHANGES ON on 03/09/20.*/
            $this->form_validation->set_rules('relation', 'Relation', 'required|in_list[S,D,W,N]');
            $rel = escape_data($this->input->post("relation"));
            if($rel=='N'){
                $this->form_validation->set_rules('relative_name', 'Father/Husband name', 'trim|min_length[3]|max_length[99]|validate_alpha_numeric_single_double_quotes_bracket_with_special_characters');
            ...
            if($_POST['is_dead_minor'] == '0'){
                ...
                else  if(isset($country_id) && !empty($country_id) && $country_id != 96 && $_POST['is_dead_minor'] == '1'){
        ...
        $this->form_validation->set_rules('party_email', 'Email', 'trim|min_length[6]|max_length[49]|valid_email');
...
//        $this->form_validation->set_rules('party_district', 'District', 'required|trim|validate_encrypted_value');
        ...
        if (!$this->form_validation->run()) {
            echo '3@@@';
            echo form_error('party_as').form_error('p_r_type') . form_error('party_name') . form_error('relation') . form_error('relative_name') . form_error('party_age') .
        ...
        $party_max = $this->Get_details_model->get_max_party_num_n_id($registration_id, $p_r_type);
        ...
        $party_age = !empty($this->input->post("party_age")) ? escape_data($this->input->post("party_age")) : NULL;
        ...
        $party_address = strtoupper(escape_data($this->input->post("party_address")));
        ...
        $party_pincode = escape_data($this->input->post("party_pincode"));
        ...
        if (isset($registration_id) && !empty($registration_id) && isset($party_id) && !empty($party_id)) {
            ...
            $status = $this->New_case_model->add_update_case_parties($registration_id, $party_details, NEW_CASE_EXTRA_PARTY, $party_id);
            ...
            if ($step >= $breadcrumb_status) {
                if ($p_r_type == 'P') {
                    ...
                    if ($extra_party_check_current_case_details > $all_party_no || $extra_party_check_current_case_details != $all_party_no) {
                        ...
                        $inserted_party_id = $this->New_case_model->add_update_case_parties($registration_id, $party_details, NEW_CASE_EXTRA_PARTY);
