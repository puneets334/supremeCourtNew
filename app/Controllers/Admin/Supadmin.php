...
class Supadmin extends BaseController {
...
//        }
        ...
        $this->load->view('admin/supadmin_dashboard');
...
//            exit(0);
//        }
        if (!isset($_POST['efil_no'])) {
            $_SESSION['MSG'] = message_show("fail", "No Efiling number given !");
            redirect('admin/supadmin/change_case_status');
            exit(0);
        }

        $_POST['efil_no'] = str_replace('-', '', trim($_POST['efil_no']));

        if (strlen(trim($_POST['efil_no'])) != 17) {
            $_SESSION['MSG'] = message_show("fail", "Invalid Efiling number !");
            redirect('admin/supadmin/change_case_status');
            exit(0);
        }

        if (preg_match('/[^0-9a-zA-Z]/i', trim($_POST['efil_no']))) {
            $_SESSION['MSG'] = message_show("fail", "Efiling Number Can only contain characters, numbers and hyphens(-) !");
            redirect('admin/supadmin/change_case_status');
            exit(0);
        }

        $dat = $this->Supadmin_model->search_status_that_can_be_changed($_POST['efil_no']);
        if (!$dat) {
            ...
            $this->session->set_userdata($change_case_session);
    ...
    public function change_case_status_main() {
        ...
        if (!isset($this->session->userdata['change_filing_number'])) {
            $_SESSION['MSG'] = message_show("fail", "No Efiling Number was set!");
            redirect('admin/supadmin/change_case_status');
            exit(0);
        }

        $data['status_that_can_be_changed'] = $this->session->userdata['change_filing_number'];
    ...
    public function final_case_status_change() {
        ...
        if (!isset($this->session->userdata['last_stage_to_be_updated'])) {
            $_SESSION['MSG'] = message_show("fail", "No Last Stage to be updated is set !");
            redirect('admin/supadmin/change_case_status_main');
            exit(0);
        }

        if ($this->session->userdata['last_stage_to_be_updated'] != $_POST['to_change_status_id']) {
            $_SESSION['MSG'] = message_show("fail", "Status to be changed is not set !");
            redirect('admin/supadmin/change_case_status_main');
            exit(0);
        }

        if ($_POST['to_change_status_id'] == NULL || $_POST['to_change_status_id'] == '') {
    ...
    public function go_live() {
        ...
        $this->load->view('templates/admin_header');
        $this->load->view('admin/go_live_view', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function check_master() {
        ...
        $this->load->view('templates/admin_header');
        $this->load->view('admin/check_master_data', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function get_master_all_data() {
        ...
        if (!isset($court_type) || empty($court_type) || !(ENABLE_FOR_HC && ENABLE_FOR_ESTAB)) {
            if (ENABLE_FOR_HC) {
    ...
    public function check_payment() {
        ...
        $this->load->view('templates/admin_header');
        $this->load->view('admin/check_payment_getway', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function get_estab_details_value() {
        ...
        if (!isset($_POST['court_type']) || empty($_POST['court_type']) || !(ENABLE_FOR_HC && ENABLE_FOR_ESTAB)) {
            if (ENABLE_FOR_HC) {
    ...
    public function add_establishment() {
        ...
        if (empty($est_id) && empty($hcid)) {
            ...
            if ($validate_nums['response'] == FALSE) {
                $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'> " . htmlentities($validate_nums['msg']['field_name'], ENT_QUOTES) . "</div>");
                redirect('admin/supadmin/establishment');
                exit(0);
            }
            ...
            if ($validate_character['response'] == FALSE) {
                $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>" . htmlentities($validate_character['msg']['field_name'], ENT_QUOTES) . "</div>");
                redirect('admin/supadmin/establishment');
                exit(0);
            }
            $validate_alpha = validate_alphanumeric($estab_id, 'Establishment id');
            if ($validate_nums['response'] == FALSE) {
                $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'> " . htmlentities($validate_alpha['msg']['field_name'], ENT_QUOTES) . "</div>");
                redirect('admin/supadmin/establishment');
                exit(0);
            }
        ...
        if (empty($payment_method)) {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>Choose any Payment Method.</div>");
            redirect('admin/supadmin/establishment');
            exit(0);
        ...
        if ($payment_mode == 1 && $_SESSION['login']['state_payment_gateway'] == HR_GRAS_PAYMENT_GATEWAY_CODE) {
            ...
            $SCHEMECOUNT = "1";
            $SCHEMENAME = HR_GRASS_PAYMENT_SCHEMENAME;
            if ($payment_mode == 1) {
            ...
            if (!empty($depcode)) {
                if (preg_match('/[^A-Za-z\s]/i', $depcode) || $depcode == NULL) {
                    ...
                    $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>Enter Valid Department Code</div>");
                    redirect('admin/supadmin/establishment');
                    exit(0);
                }
            }
        ...
        if ($payment_mode == 1 && $_SESSION['login']['state_payment_gateway'] == MH_GRAS_PAYMENT_GATEWAY_CODE) {
            ...
            $subsystem = MH_GRAS_PAYMENT_SUBSYSTEM; //'Principaljudge';
            ...
            if (!preg_match("/^[A-Za-z]*$/", $dept) || strlen($dept) < 3 || strlen($dept) > 3 || empty($dept)) {
                $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>" . htmlentities("Enter Valid Department Code.Length Only 3 Characters.", ENT_QUOTES) . "</div>");
                redirect('admin/supadmin/establishment');
                exit(0);
            }

            if (preg_match("/[^a-z_\-0-9]/i", $tcode) || strlen($tcode) > 4 || empty($tcode)) {
                $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>" . htmlentities("Enter Valid Treasury Code.Length Only 4 Characters.", ENT_QUOTES) . "</div>");
                redirect('admin/supadmin/establishment');
                exit(0);
            }
            ...
            if (preg_match("/[^a-z_\-0-9]/i", $office_code) || strlen($office_code) > 6 || strlen($office_code) < 6 || empty($office_code)) {
                ...
                $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>" . htmlentities("Enter Valid Office Code.Length Only 6 Characters.", ENT_QUOTES) . "</div>");
                redirect('admin/supadmin/establishment');
                exit(0);
            }
            ...
            if (preg_match('/[^0-9a-zA-Z\s.&_]/i', $major) || strlen($major) > 4 || strlen($major) < 4 || empty($major)) {
                ...
                $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'> Enter Valid Major Head. Max 4 Characters.</div>");
                redirect('admin/supadmin/establishment');
                exit(0);
            }

            if (preg_match("/[^a-z_\-0-9-]/i", $hoa1) || strlen($hoa1) > 16 || empty($hoa1)) {
                $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'> " . htmlentities("Enter Valid HOA1.10 digit scheme code Hyphen 11th Position 12 th and 13th will be Object head. ", ENT_QUOTES) . "</div>");
                redirect('admin/supadmin/establishment');
                exit(0);
            }
        ...
        if ($payment_mode == 1 && $_SESSION['login']['state_payment_gateway'] == SHCIL_PAYMENT_GATEWAY_CODE) {

            if (ENABLE_FOR_HC == TRUE && !empty($hc_code)) {
                $mjhead = 'HC-Fee';
                $shead = trim(strtoupper($hc_name));
                $smjhead = "";
                $minhead = $hc_code;
            } else if (ENABLE_FOR_ESTAB == TRUE) {
                $mjhead = 'DC-Fee';
...
//$shead = trim($estab_name.','.$dist_name);
            ...
            $payment_parameter->login = STOCK_HOLDING_LOGIN;
            $payment_parameter->password = STOCK_HOLDING_PASSWORD;
            $payment_parameter->Product = STOCK_HOLDING_PRODUCT;
            $payment_parameter->ReqHashKey = STOCK_HOLDING_REQHASHKEY;
            $payment_parameter->RespHashKey = STOCK_HOLDING_RESPHASHKEY;
            $payment_parameter->txnTyp = 'NB';
            $payment_parameter->scamt = "0";
            $payment_parameter->udf1 = STOCK_HOLDING_UDF1;
            $payment_parameter->udf2 = STOCK_HOLDING_UDF2;
            $payment_parameter->udf3 = STOCK_HOLDING_UDF3;
            $payment_parameter->udf4 = STOCK_HOLDING_UDF4;
            $payment_parameter->udf5 = STOCK_HOLDING_UDF5;
        ...
        if (empty($cde_add) && empty($efil_add)) {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'> Choose any Check box (Efiling or CDE).</div>");
            redirect('admin/supadmin/establishment');
            exit(0);
        }
        if (strlen($appip) > 15) {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>Enter valid ip address</div>");
            redirect('admin/supadmin/establishment');
            exit(0);
        }
        ...
        if ($payment_mode == 1) {
            if ($_SESSION['login']['state_payment_gateway'] == SBI_PAYMENT_GATEWAY_CODE) {
                $gateway_code = SBI_PAYMENT_GATEWAY_CODE;
            } elseif ($_SESSION['login']['state_payment_gateway'] == MH_GRAS_PAYMENT_GATEWAY_CODE) {
                $gateway_code = MH_GRAS_PAYMENT_GATEWAY_CODE;
            } elseif ($_SESSION['login']['state_payment_gateway'] == HR_GRAS_PAYMENT_GATEWAY_CODE) {
                $gateway_code = HR_GRAS_PAYMENT_GATEWAY_CODE;
            } elseif ($_SESSION['login']['state_payment_gateway'] == SHCIL_PAYMENT_GATEWAY_CODE) {
                $gateway_code = SHCIL_PAYMENT_GATEWAY_CODE;
            } elseif ($_SESSION['login']['state_payment_gateway'] == CG_GRAS_PAYMENT_GATEWAY_CODE) {
                $gateway_code = CG_GRAS_PAYMENT_GATEWAY_CODE;
            }
        } else {
            $gateway_code = '';
        }
        if (empty($hcid)) {
            ...
            if (empty($est_id)) {
                ...
                if (!$get_estb) {
                    $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'> Establishment Already Exist.</div>");
                    redirect('admin/supadmin/establishment');
                } else {
                    $result = $this->Supadmin_model->add_establishment_detail($state_id, $dist_code, $distdata, $estabdata);
                    if ($result) {
                        $this->session->set_flashdata('message', "<div class='alert alert-success fade in'> Establishment Inserted Successfully.</div>");
                        redirect('admin/supadmin/establishment');
                    } else {
                        $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'> Some thing wrong! Please try again later.</div>");
                        redirect('admin/supadmin/establishment');
                    }
                }
            } else {
        ...
        if (!empty($est_id)) {
            ...
            if ($result) {
                $this->session->set_flashdata('message', "<div class='alert alert-success fade in'> Establishment Updated Successfully.</div>");
                redirect('admin/supadmin/establishment');
            } else {
                $this->session->set_flashdata('message', "<div class='text-danger'> Some thing wrong! Please try again later.</div>");
                redirect('admin/supadmin/establishment');
            }
        }if (!empty($hcid)) {
            $result = $this->Supadmin_model->update_hc_detail($hcid, $updatestabdata);
            if ($result) {
                $this->session->set_flashdata('message', "<div class='alert alert-success fade in'> High court Updated Successfully.</div>");
                redirect('admin/supadmin/establishment');
            } else {
                $this->session->set_flashdata('message', "<div class='text-danger'> Some thing wrong! Please try again later.</div>");
                redirect('admin/supadmin/establishment');
    ...
    public function action($action_id, $status) {
        ...
        if (preg_match('/[^0-9]/i', $action_id)) {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>Something Wrong! Please Try again letter1.</div>");
            redirect('admin/supadmin/establishment');
        }
    ...
    function admin_list() {
        ...
        $this->load->view('templates/admin_header');
        $this->load->view('admin/adv_admin_list', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function department() {
        ...
        $this->load->view('templates/header');
        $this->load->view('department/add_department_view', $data);
        $this->load->view('templates/footer');
    }
