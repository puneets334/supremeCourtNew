...
use App\Models\admin\NocVakalatnama_model;


class Noc_vakalatnama extends BaseController
{
    ...
    public function __construct() {
        parent::__construct();
        $this->NocVakalatnama_model = new NocVakalatnama_model();
        ...
        if(isset($_POST['e_filing_num']) && !is_null($_POST['e_filing_num']))
        {
            ...
            if(isset($_POST['registration_id'], $_POST['new_aor']) && !is_null($_POST['registration_id']) && !is_null($_POST['new_aor'])){                    
              $result= $this->NocVakalatnama_model->update_aor_in_case(trim($_POST['registration_id']), $_POST['new_aor']);
                if ($result)
                    $this->session->setFlashdata('message', "<div class='text-success'>Status Updated Successfully .</div>");
                else
                    $this->session->setFlashdata('message', "<div class='text-danger'>Something Wrong! Please Try again letter.</div>");
