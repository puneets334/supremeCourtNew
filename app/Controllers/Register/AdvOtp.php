...
class AdvOtp extends BaseController {
    ...
    public function index() {
        ...
        //print_r($_SESSION['adv_details']);exit();
    ...
    function verify(){
        ...
        $this->form_validation->set_rules('adv_email_otp', 'Email OTP', 'required|numeric|min_length[6]|max_length[6]|trim|is_required');
            $this->session->set_flashdata('msg', '<div class="uk-alert-danger flashmessage" uk-alert> <a class="uk-alert-close " uk-close></a > <p style="text-align: center;">Mobile OTP And Email OTP Required</p> </div>');
    }
    	 ...
    	 if ($this->form_validation->run() == FALSE) { 
        	...
        	if(isset($_POST['adv_mobile_otp']) && !empty($_POST['adv_mobile_otp']) || isset($_POST['adv_email_otp']) && !empty($_POST['adv_email_otp'])){
        		...
        		$_SESSION['verify_details'] =  array('mobile_verified'=>$mobile_status,
        		 'email_verified'=>$email_status);
                 if ($mobile_status == 'expired' || $email_status == 'expired'){
                    $captcha_value = captcha_generate();
                    $data['captcha']['image'] = $captcha_value['image'];
                    $data['captcha']['word'] = $captcha_value['word'];
                    $this->session->set_flashdata('msg', '<div class="uk-alert-danger flashmessage" uk-alert> <a class="uk-alert-close " uk-close></a > <p style="text-align: center;">OTP Expired.</p> </div>');
                    $this->render('responsive_variant.authentication.adv_otp_view',$data);
                }
                else if($mobile_status == 'done' && $email_status == 'done'){
                    $this->session->set_flashdata('msg', '<div class="uk-alert-success flashmessage" uk-alert> <a class="uk-alert-close " uk-close></a > <p style="text-align: center;">OTP Verification Successful !</p> </div>');
                    if(!empty($_SESSION['adv_details']['register_type']) && $_SESSION['adv_details']['register_type'] == 'Advocate'){
                        ...
                        redirect('saveArguingCounselCompleteDetails');
                    }
                    else{
                    ...
                    $data['captcha']['image'] = $captcha_value['image'];
                    $data['captcha']['word'] = $captcha_value['word'];
                    $this->session->set_flashdata('msg', '<div class="uk-alert-danger flashmessage" uk-alert> <a class="uk-alert-close " uk-close></a > <p style="text-align: center;">OTP Verification Failed</p> </div>');
                    $this->render('responsive_variant.authentication.adv_otp_view',$data);
                }
