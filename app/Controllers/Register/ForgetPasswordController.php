...
class ForgetPasswordController extends BaseController
 {
	...
	public function __construct() {
        ...
        $this->Register_model = model('App\Models\Register\Register_model');
    ...
    public function adv_get_otp()
    {
        ...
        if ($this->validate($rules) === FALSE) {
            ...
            return view('responsive_variant.authentication.forget_password_view', $data);
        }
        ...
        if (empty($adv_mobile) && empty($adv_email)) {
            $this->session->setFlashdata('msg', '<div class="uk-alert-danger" uk-alert><a class="uk-alert-close" uk-close></a><p style="text-align: center;">Please Enter Mobile or Email!</p></div>');
    
            return redirect()->to(base_url('Register/ForgetPassword'));
        }
    
        // Check captcha validity
        if ($_SESSION["captcha"] != $this->request->getPost('userCaptcha')) {
        ...
        if (!$mobile_exist || !$email_exist) {
            $this->session->setFlashdata('msg', '<div class="uk-alert-danger" uk-alert><a class="uk-alert-close" uk-close></a><p style="text-align: center;">Not Registered With This Mobile Number or Email ID!</p></div>');
...
{
    if (isset($_SESSION['adv_details'][$sessionOtpKey]) && $_SESSION['adv_details'][$sessionOtpKey] == $this->request->getPost($postOtpKey)) {
        if ($currentTime <= strtotime($_SESSION['adv_details']['end_time'])) {
            return true;
        }
    }
    return false;
    ...
    public function AdvSignUp()
    {
        ...
        $mobileNo = $session->get('adv_details')['mobile_no'];
        $emailId = $session->get('adv_details')['email_id'];
