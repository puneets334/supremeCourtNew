    ...
    public function get_case_status_other_tab_data()
    {
        ...
        $case_status_tab_content='';
         if(!empty($_POST["diaryno"]) && !empty($_POST["according_id"]))
         {
            ...
            $diary_no = escape_data($this->input->post("diaryno"));
            $accordingId = escape_data($this->input->post("according_id"));
            $accordingNt = escape_data($this->input->post("according_nt"));

            //echo $accordingId.'#'.$accordingNt.'<br>';
            ...
            // echo CASE_STATUS_ADDON_API.$url;
             //exit();
