...
class DefaultController extends BaseController
{
    ...
    //     unset($_SESSION['data_arrays']);
    //     if ($this->session->userdata['login']) {
    //         $data['AOR']=array();
    //         $aor_code=(int)$this->session->userdata['login']['aor_code'];
    //         if (!empty($aor_code)){
    ...
    //         $data['account_status'] = $this->Common_login_model->get_account_status($_SESSION['login']['userid']);
    //         $data['show_estab'] = $this->Common_login_model->show_estab_profile($this->session->userdata['login']['id']);
    //         if ($_SESSION['login']['ref_m_usertype_id'] == USER_IN_PERSON || $_SESSION['login']['ref_m_usertype_id'] == USER_ADVOCATE || $_SESSION['login']['ref_m_usertype_id'] == USER_DEPARTMENT || $_SESSION['login']['ref_m_usertype_id'] == USER_CLERK || $_SESSION['login']['ref_m_usertype_id'] == SR_ADVOCATE) {
    //             $this->load->view('templates/header');
    //         } else {
    //             $this->load->view('templates/admin_header');
    //         }
    //         //echo '<pre>'; print_r($data['profile']); echo '</pre>'; exit();
    ...
    public function index()
    {
        ...
        if ($session->get('login')) {
            ...
            //     $AORDetails_data = $client->request('GET', ICMIS_SERVICE_URL . '/ConsumedData/getAORDetails/?aor_code=' . $aor_code);
            //     $AORDetails = json_decode($AORDetails_data->getBody());
            //     if (!empty($AORDetails->data)) {
            //         $data['AOR'] = (array) $AORDetails->data[0];
            //     }
            // }
            ...
            $Common_login_model = new CommonLoginModel();
            $data['high_court_list'] = $Dropdown_list_model->get_high_court_list();
            $data['profile'] = $Profile_model->getProfileDetail($session->get('login')['userid']);
//pr($data['profile'] );
            //$data['state_list'] = $Dropdown_list_model->get_state_list_Y();
            //$data['account_status'] = $Common_login_model->get_account_status($session->get('login')['userid']);
            //$data['show_estab'] = $Common_login_model->show_estab_profile($session->get('login')['id']);
            ...
            $header_view = ($_SESSION['login']['ref_m_usertype_id'] == USER_IN_PERSON ||
                $_SESSION['login']['ref_m_usertype_id'] == USER_ADVOCATE ||
                $_SESSION['login']['ref_m_usertype_id'] == USER_DEPARTMENT ||
                $_SESSION['login']['ref_m_usertype_id'] == USER_CLERK ||
                $_SESSION['login']['ref_m_usertype_id'] == SR_ADVOCATE) ? 'templates/header' : 'templates/admin_header';
    ...
    // public function updateProfile($get)
    // {
    ...
    //     if ($this->session->userdata['login']) {
    //         $data['updatedata'] = $get;
    //         if (!in_array($get, array('email', 'contact', 'other', 'address', 'pass', 'estab'))) {
    //             redirect('profile');
    //         }
    //         $data['state_list'] = $this->Register_model->get_state_list();
    //         $data['profile'] = $this->Profile_model->getProfileDetail($this->session->userdata['login']['userid']);
    //         //$data['district'] =$this->Register_model->get_district_list($data['profile'][0]->m_state_id);
    //         if ($_SESSION['login']['ref_m_usertype_id'] == USER_IN_PERSON || $_SESSION['login']['ref_m_usertype_id'] == USER_ADVOCATE || $_SESSION['login']['ref_m_usertype_id'] == USER_DEPARTMENT || $_SESSION['login']['ref_m_usertype_id'] == USER_CLERK || $_SESSION['login']['ref_m_usertype_id'] == SR_ADVOCATE) {
    //             $this->load->view('templates/header');
    //         } else {
    //             $this->load->view('templates/admin_header');
    ...
    public function updateProfile($param)
    {
        ...
        // Assuming captcha_generate is a helper function, load the helper
        ...
        if ($session->get('login')) {
            ...
            $data['state_list'] = $registerModel->get_state_list();
            $data['profile'] = $profileModel->getProfileDetail($session->get('login')['userid']);


            // Load header view based on user type
            // if (in_array($session->get('login')['ref_m_usertype_id'], [USER_IN_PERSON, USER_ADVOCATE, USER_DEPARTMENT, USER_CLERK, SR_ADVOCATE])) {
            //     echo view('templates/header');
            // } else {
            //     echo view('templates/admin_header');
            // }

            // Generate and set a random string for session
    ...
    //     if ($this->session->userdata['login']) {
    //         $currentpass = escape_data($this->input->post("currentpass"));
    //         $newpass = escape_data($this->input->post("newpass"));
    //         $confirmpass = escape_data($this->input->post("confirmpass"));
    //         $x = explode($_SESSION['login_salt'],$newpass);
    //         $y = explode($_SESSION['login_salt'],$confirmpass);
    //         $newpass = $x[0];
    //         $confirmpass = $y[0];
    //         //set validations
    ...
    //         $this->form_validation->set_rules("confirmpass", "Confirm New Password", "trim|required");
    ...
    //                 $this->session->set_flashdata('msg', '<div class="alert alert-danger alert-dismissible text-center flashmessage">Password policy has to be followed.</div>');
    //                 redirect('profile/DefaultController/updateProfile/pass');
    //             }


    //             //validation succeeds
    //                 $current_password = $this->Profile_model->selectPassword($this->session->userdata['login']['userid']);

    //                     if ($current_password[0]->password . $_SESSION['login_salt'] == $currentpass) {

    //                         if ($newpass . $_SESSION['login_salt'] != $password) {
    //                     if ($newpass == $confirmpass) {

    //                         $resultCheck=$this->Profile_model->selectPasswordCheck($this->session->userdata['login']['id'], $newpass);
    //                         if ($resultCheck==false){

    //                             $this->Profile_model->updatePasswordCheck($this->session->userdata['login']['id'], $newpass);
    //                             $this->Profile_model->updatePassword($this->session->userdata['login']['userid'], $newpass);
    //                             $this->session->set_flashdata('msg', '<div class="alert alert-success alert-dismissible text-center flashmessage">Password Updated!</div>');

    //                             //redirect('profile/DefaultController/updateProfile/pass');
    //                             redirect('login/logout');
    //                         }else{

    //                             $this->session->set_flashdata('msg', '<div class="alert alert-danger text-center flashmessage" data-auto-dismiss="9000">Entered new password was already used earlier, please enter different password.</div>');
    //                             redirect('profile/DefaultController/updateProfile/pass');
    //                         }

    //                     } else {

    //                         $this->session->set_flashdata('msg', '<div class="alert alert-danger alert-dismissible text-center flashmessage">Confirm Password does not match!</div>');
    //                         redirect('profile/DefaultController/updateProfile/pass');
    //                     }
    //                     }else{

    //                         $this->session->set_flashdata('msg', '<div class="alert alert-danger text-center flashmessage" data-auto-dismiss="9000">New password should not be same as current password!</div>');
    //                         redirect('profile/DefaultController/updateProfile/pass');
    //                     }
    //                 } else {

    //                     $this->session->set_flashdata('msg', '<div class="alert alert-danger alert-dismissible text-center flashmessage">Invalid Current Password!</div>');
    //                     redirect('profile/DefaultController/updateProfile/pass');
    //                 }
    ...
    //         $validation->setRules([
    //             'currentpass' => 'trim|required',
    //             'newpass' => 'trim|required',
    //             'confirmpass' => 'trim|required',
    //         ]);
    ...
    //             $current_password = $profileModel->selectPassword($session->get('login')['userid']);

    //             if ($current_password[0]->password . $session->get('login_salt') == $currentpass) {
    //                 if ($newpass . $session->get('login_salt') != $password) {
    //                     if ($newpass == $confirmpass) {

    //                         $resultCheck = $profileModel->selectPasswordCheck($session->get('login')['id'], $newpass);
    //                         if (!$resultCheck) {
    //                             $profileModel->updatePasswordCheck($session->get('login')['id'], $newpass);
    //                             $profileModel->updatePassword($session->get('login')['userid'], $newpass);
    ...
    //             $data['profile'] = $profileModel->getProfileDetail($session->get('login')['userid']);
    //             $data['updatedata'] = 'pass';
    //             // if ($session->get('login')['ref_m_usertype_id'] == USER_IN_PERSON || $session->get('login')['ref_m_usertype_id'] == USER_ADVOCATE || $session->get('login')['ref_m_usertype_id'] == USER_DEPARTMENT || $session->get('login')['ref_m_usertype_id'] == USER_CLERK) {
    //             //     echo view('templates/header');
    //             // } else {
    //             //     echo view('templates/admin_header');
    //             // }
    //             // echo view('profile_update_view', $data);
    ...
    public function updatePass()
{
    ...
    {
        if ($this->session->userdata['login']) {
            $emailid = escape_data($this->input->post("emailid"));
            $email_pattern = "/^[_a-zA-Z0-9-]+(\.[_a-zA-Z0-9-]+)*@[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*(\.[a-zA-Z]{2,})$/";

            if (!preg_match($email_pattern, $emailid)) {
                $this->session->set_flashdata('msg', '<div class="alert alert-danger alert-dismissible text-center flashmessage">Wrong email address !!</div>');
            ...
            if ($check_email) {
                $this->session->set_flashdata('msg', '<div class="alert alert-danger alert-dismissible text-center flashmessage">This email id already registered !</div>');
                redirect('profile/DefaultController/updateProfile/email');
            }
            //set validations
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if (isset($_POST['update_email'])) {
                    ...
                    if ($previous_emailid[0]->emailid != $emailid) {
                        ...
                        $this->session->set_userdata('email_id_for_updation', $emailid);
                        ...
                        $this->session->set_flashdata('msg', '<div class="alert alert-danger alert-dismissible text-center flashmessage">Enter different email Id!</div>');
                        redirect('profile');
                    }
                }
            } else {
    ...
    {
        if ($this->session->userdata['login']) {
            $mobile_number = escape_data($this->input->post("moblie_number"));
            if (!isset($mobile_number)) {
                redirect('login');
            }
            if (!preg_match("/^[0-9]{10}$/", $mobile_number)) {
                $this->session->set_flashdata('msg', '<div class="alert alert-danger alert-dismissible text-center flashmessage">Wrong contact number !!</div>');
            ...
            if ($check_mobno) {
                $this->session->set_flashdata('msg', '<div class="alert alert-danger alert-dismissible text-center flashmessage">This mobile number already registered !</div>');
                redirect('profile/DefaultController/updateProfile/contact');
            }
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if (isset($_POST['moblie_number'])) {
                    ...
                    if ($previous_mobile_number[0]->moblie_number != $mobile_number) {
                        ...
                        $this->session->set_userdata('login', $login);
                        $this->session->set_userdata('mobile_no_for_updation', $mobile_number);
                        ...
                        $this->session->set_flashdata('msg', '<div class="alert alert-danger alert-dismissible text-center flashmessage">Enter different Contact!</div>');
                        redirect('profile');
                    }
                }
            } else {
    ...
    {
        if ($this->session->userdata['login']) {
            $other_contact_number = escape_data($this->input->post("other_contact_number"));
            if (!preg_match("/^[0-9]{10}$/", $other_contact_number)) {
                $this->session->set_flashdata('msg', '<div class="alert alert-danger alert-dismissible text-center flashmessage">Other contact number should be 10 digit !</div>');
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if (isset($_POST['update_other'])) {
                    ...
                    if ($previous_other_contact_number[0]->other_contact_number != $other_contact_number) {
                        ...
                        $this->session->set_flashdata('msg', '<div class="alert alert-success alert-dismissible text-center flashmessage">Other Contact No. Updated successfully !</div>');
                        redirect('profile');
                    } else {
                        $this->session->set_flashdata('msg', '<div class="alert alert-danger alert-dismissible text-center flashmessage">Enter different Contact!</div>');
                        redirect('profile/DefaultController/updateProfile/other');
    ...
    public function updateAddress()
    {
        // print_r($_POST);die;
        if ($this->session->userdata['login']) {
            $data['address1'] = escape_data($this->input->post("address1"));
            $data['address2'] = escape_data($this->input->post("address2"));
            $data['city'] = escape_data($this->input->post("city"));
            $data['pincode'] = escape_data($this->input->post("pincode"));
            if (!empty($this->input->post("ref_m_states_id"))) {
                $data['ref_m_states_id'] = escape_data(url_decryption($this->input->post("ref_m_states_id")));
            } else {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $this->session->set_flashdata('msg', '<div class="alert alert-success alert-dismissible text-center flashmessage">Address Updated successfully !</div>');
                redirect('profile');
            } else {
                $data['profile'] = $this->Profile_model->getProfileDetail($this->session->userdata['login']['userid']);
    ...
    public function emailSave()
    {
        if ($this->session->userdata['login']) {
            if (isset($_POST['email_save'])) {
                ...
                $emailid = escape_data($this->session->userdata('email_id_for_updation'));
                $otp = escape_data($this->input->post("email_otp"));
                if (!preg_match("/^[0-9]*$/", $otp)) {
                    $this->session->set_flashdata('msg', '<div class="alert alert-danger alert-dismissible text-center flashmessage">Wrong OTP !!</div>');
                    redirect('profile');
                }
                if ($otp == $this->session->userdata['login']['email_otp']) {
                    ...
                    $this->session->unset_userdata('email_id_for_updation');
                    $this->session->set_flashdata('msg', '<div class="alert alert-success alert-dismissible text-center flashmessage">Email Updated successfully !</div>');
                    redirect('profile');
                } else {
                    $this->session->set_flashdata('msg', '<div class="alert alert-danger alert-dismissible text-center flashmessage">Invalid OTP!</div>');
                    redirect('profile');
                }
            }
        } else {
    ...
    public function mobileSave()
    {
        if ($this->session->userdata['login']) {
            if (isset($_POST['mobile_save'])) {
                $mobile_number = escape_data($this->session->userdata('mobile_no_for_updation'));
                ...
                if ($otp == $this->session->userdata['login']['mobile_otp']) {
                    ...
                    if ($result) {
                        ...
                        $this->session->unset_userdata('mobile_no_for_updation');
                        $this->session->set_flashdata('msg', '<div class="alert alert-success alert-dismissible text-center flashmessage">Contact Updated successfully!</div>');
                        redirect('profile');
                    } else {
                        $this->session->set_flashdata('msg', '<div class="alert alert-danger alert-dismissible text-center flashmessage">Updation Failed! Contact may be exist!</div>');
                        redirect('profile');
                    }
                } else {
                    $this->session->set_flashdata('msg', '<div class="alert alert-danger alert-dismissible text-center flashmessage">Invalid OTP!</div>');
    ...
    function updateProfile_adv()
    {
        ...
        if ($update_profile) {
            ...
            $this->session->set_flashdata('msg_objection', '<div class="alert alert-success text-center flashmessage">Profile photo Updated Successful!</div>');
            redirect('profile');
        }
    }
    ...
    function logged_profile()
    {
        if ($this->session->userdata['login']) {
            ...
            $this->load->view('profile/logged_profile', $data);
            $this->load->view('templates/footer');
        } else {
            redirect('login');
        }
    ...
    function block_user()
    {
        ...
        if (empty($ip) || empty($userid) || empty($status)) {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>Something Wrong! Please Try again letter.</div>");
        ...
        if ($result) {
            $this->session->set_flashdata('message', "<div class='alert alert-success fade in'>Status Updated Successfully .</div>");
            redirect('profile/logged_profile');
        } else {
            $this->session->set_flashdata('message', "<div class='alert alert-danger fade in'>Something Wrong! Please Try again letter.</div>");
            redirect('profile/logged_profile');
