...
class DefaultController extends BaseController
{
    ...
    public function index($id = NULL)
    {
        ...
        if ($id) {
            ...
            if (!is_numeric($InputArrray[0]) || $InputArrray[1] != E_FILING_TYPE_CERTIFICATE_REQUEST || !is_numeric($InputArrray[2])) {
                redirect('login');
                exit(0);
            }
            ...
            $estab_details = $this->Certificate_model->get_establishment_details();

            if ($estab_details) {

                $efiling_num_details = $this->Certificate_model->get_efiling_num_basic_Details($registration_id);
            ...
            $estab_details = $this->Certificate_model->get_establishment_details();
            if ($estab_details) {
                $_SESSION['efiling_details']['ref_m_efiled_type_id'] = E_FILING_TYPE_CERTIFICATE_REQUEST;
            ...
            if ($_SESSION['login']['ref_m_usertype_id'] == USER_ADMIN || !in_array($_SESSION['efiling_details']['stage_id'], $stages_array)) {
                ...
                if (in_array($_SESSION['login']['ref_m_usertype_id'], $allowed_users)) {
                    ...
                    switch (max(explode(',', $_SESSION['efiling_details']['breadcrumb_status']))) {
                        ...
                        case CERTIFICATE_BREAD_REQUEST_DETAILS:
                            redirect('certificate/AddDetails');
                        default:
