<?php
if ($this->uri->segment(3) == 'total') {
    $result = $total_matters_data;
    $title = "Total Matters";
} elseif ($this->uri->segment(3) == 'pending') {
    $result = $pending_matters_data;
    $title = "Pending Matters";
} elseif ($this->uri->segment(3) == 'pending_reg') {
    $result = $pending_reg_matters_data;
    $title = "Pending Registered Matters";
} elseif ($this->uri->segment(3) == 'pending_unreg') {
    $result = $pending_unreg_matters_data;
    $title = "Pending Unregistered Matters";
} elseif ($this->uri->segment(3) == 'disposed') {
    $result = $disposed_matters_data;
    $title = "Disposed Matters";
} elseif ($this->uri->segment(3) == 'pending_reg_pet') {
    $result = $pending_reg_pet_matters_data;
    $title = "Pending Registered Petitioner Matters";
} elseif ($this->uri->segment(3) == 'pending_reg_res') {
    $result = $pending_reg_res_matters_data;
    $title = "Pending Registered Respondent Matters";
} elseif ($this->uri->segment(3) == 'pending_unreg_pet') {
    $result = $pending_unreg_pet_matters_data;
    $title = "Pending Unregistered Petitioner Matters";
} elseif ($this->uri->segment(3) == 'pending_unreg_res') {
    $result = $pending_unreg_res_matters_data;
    $title = "Pending Unregistered Respondent Matters";
} elseif ($this->uri->segment(3) == 'pending_reg_other') {
    $result = $pending_reg_other_matters_data;
    $title = "Pending Registered Other Matters";
} elseif ($this->uri->segment(3) == 'pending_unreg_other') {
    $result = $pending_unreg_other_matters_data;
    $title = "Pending unregistered Other Matters";
} else {
    $result = $pending_reg_matters_data;
    $title = "Pending Registered Matters";
}
    ...
    <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="col-md-7 col-sm-12 col-xs-12">
            <div class="row">
                <div class="form-group">
                    <label class="control-label col-md-8 col-sm-12 col-xs-12 input-sm" style="font-size: 18px;"><?php echo echo_data($title); ?>:</label>
        ...
        <!--        <div class="col-md-5 col-sm-12 col-xs-12">
                    <div class="col-md-6 col-sm-12 col-xs-12">
                        <div class="form-group">
                            <select class="form-control input-md filter_select_dropdown" id="case_year" name="case_year" > 
        ...
        <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
            <thead>
                <tr class="success input-sm" role="row" >
                    <th  width="5%"> # </th>
                    <th width="20%"> Diary No / Registration No.</th>
            ...
            <tbody>	
                ...
                foreach ($result as $data) {
                    ...
                    if (!empty($data['reg_no_display'])) {
                        $reg_no_display = htmlentities($data['reg_no_display'], ENT_QUOTES);
                        $reg_no_whats = '*REG. NO* : ' . htmlentities($data['reg_no_display'], ENT_QUOTES) . '%0A';
                    }
                    $cause_titles = $reg_no_display . '<br>' . $pet_name . '<b> Vs. </b> ' . $res_name;

                    if (!empty($data['diary_date'])) {

                        $diary_date = '<b>Diary Date :</b>' . htmlentities($data['diary_date'], ENT_QUOTES);
                        $diary_date_mail = '<b>DIARY DATE : </b>' . date("d-m-Y", strtotime(htmlentities($data['diary_date'], ENT_QUOTES))) . '<br>';
                        $diary_date_whats = '*DIARY DATE* : ' . date("d-m-Y", strtotime(htmlentities($data['diary_date'], ENT_QUOTES))) . '%0A';
                    }

                    if (!empty($data['next_date'])) {
                        $next_date = '<br><b>Next Date :</b>' . htmlentities($data['next_date'], ENT_QUOTES);
                        $next_date_mail = '<b>NEXT DATE : </b>' . date("d-m-Y", strtotime(htmlentities($data['next_date'], ENT_QUOTES))) . '<br>';
                        $next_date_whats = '*NEXT DATE* : ' . date("d-m-Y", strtotime(htmlentities($data['next_date'], ENT_QUOTES))) . '%0A';
                    }

                    if (!empty($data['defects_notified_date'])) {
                        $defects_notified_date = '<br><b>Defects Notified :</b>' . htmlentities($data['defects_notified_date'], ENT_QUOTES);
                        $defects_notified_date_mail = '<b>DEFECTS NOTIFIED DATE : </b>' . date("d-m-Y", strtotime(htmlentities($data['defects_notified_date'], ENT_QUOTES))) . '<br>';
                        $defects_notified_date_whats = '*DEFECTS NOTIFIED DATE* : ' . date("d-m-Y", strtotime(htmlentities($data['defects_notified_date'], ENT_QUOTES))) . '%0A';
                    }


                    if (!empty($data['disposal_date'])) {
                        $disposal_date = '<br><b>Disposal Date :</b>' . htmlentities($data['disposal_date'], ENT_QUOTES);
                        $disposal_date_mail = '<b>DISPOSAL DATE : </b>' . date("d-m-Y", strtotime(htmlentities($data['disposal_date'], ENT_QUOTES))) . '<br>';
                        $disposal_date_whats = '*DISPOSAL DATE* : ' . date("d-m-Y", strtotime(htmlentities($data['disposal_date'], ENT_QUOTES))) . '%0A';
                    }


                    if (!empty($data['order_date'])) {
                        $order_date = '<br><b>Order Date :</b>' . htmlentities($data['order_date'], ENT_QUOTES);
                        $order_date_mail = '<b>ORDER DATE : </b>' . date("d-m-Y", strtotime(htmlentities($data['order_date'], ENT_QUOTES))) . '<br>';
                        $order_date_whats = '*ORDER DATE* : ' . date("d-m-Y", strtotime(htmlentities($data['order_date'], ENT_QUOTES))) . '%0A';
                    }

                    if (!empty($data['last_order_date'])) {
                        $last_order_date = '<br><b>Last Order Date :</b>' . htmlentities($data['last_order_date'], ENT_QUOTES);
                        $last_date_mail = '<b>LAST ORDER DATE : </b>' . date("d-m-Y", strtotime(htmlentities($data['last_order_date'], ENT_QUOTES))) . '<br>';
                        $last_date_whats = '*LAST ORDER DATE* : ' . date("d-m-Y", strtotime(htmlentities($data['last_order_date'], ENT_QUOTES))) . '%0A';
                    }

                    if (!empty($data['refiled_date'])) {
                        $refiled_date = '<br><b>Refiled :</b>' . htmlentities($data['refiled_date'], ENT_QUOTES);
                        $refiled_date_mail = '<b>REFILED DATE : </b>' . date("d-m-Y", strtotime(htmlentities($data['refiled_date'], ENT_QUOTES))) . '<br>';
                        $refiled_date_whats = '*REFILED DATE* : ' . date("d-m-Y", strtotime(htmlentities($data['refiled_date'], ENT_QUOTES))) . '%0A';
                    }

                    if (!empty($data['applied_date'])) {
                        $applied_date = '<br><b>Applied Date :</b>' . htmlentities($data['applied_date'], ENT_QUOTES);
                        $applied_date_mail = '<b>APPLIED DATE : </b>' . date("d-m-Y", strtotime(htmlentities($data['applied_date'], ENT_QUOTES))) . '<br>';
                        $applied_date_whats = '*APPLIED DATE* : ' . date("d-m-Y", strtotime(htmlentities($data['applied_date'], ENT_QUOTES))) . '%0A';
                    }

                    if (!empty($data['ready_date'])) {
                        $ready_date = '<br><b>Ready Date :</b>' . htmlentities($data['ready_date'], ENT_QUOTES);
                        $ready_date_mail = '<b>READY DATE : </b>' . date("d-m-Y", strtotime(htmlentities($data['ready_date'], ENT_QUOTES))) . '<br>';
                        $ready_date_whats = '*READY DATE* : ' . date("d-m-Y", strtotime(htmlentities($data['ready_date'], ENT_QUOTES))) . '%0A';
                    }

                    if (!empty($data['delivery_date'])) {
                        $delivery_date = '<br><b>Delivery Date :</b>' . htmlentities($data['delivery_date'], ENT_QUOTES);
                        $delivery_date_mail = '<b>DELIVERY DATE : </b>' . date("d-m-Y", strtotime(htmlentities($data['delivery_date'], ENT_QUOTES))) . '<br>';
                        $delivery_date_whats = '*DELIVERY DATE* : ' . date("d-m-Y", strtotime(htmlentities($data['delivery_date'], ENT_QUOTES))) . '%0A';
                    }

                    if (!empty($data['doc_filing_date'])) {
                        $doc_filing_date = '<br><b>Filing Date :</b>' . htmlentities($data['doc_filing_date'], ENT_QUOTES);
                        $doc_filing_date_mail = '<b>FILING DATE : </b>' . date("d-m-Y", strtotime(htmlentities($data['doc_filing_date'], ENT_QUOTES))) . '<br>';
                        $doc_filing_date_whats = '*FILING DATE* : ' . date("d-m-Y", strtotime(htmlentities($data['doc_filing_date'], ENT_QUOTES))) . '%0A';
                    }

                    if (!empty($data['caveat_date'])) {
                        $caveat_date = '<br><b>Caveat Date :</b>' . htmlentities($data['caveat_date'], ENT_QUOTES);
                        $caveat_date_date_mail = '<b>CAVEAT DATE : </b>' . date("d-m-Y", strtotime(htmlentities($data['caveat_date'], ENT_QUOTES))) . '<br>';
                        $caveat_date_date_whats = '*CAVEAT DATE* : ' . date("d-m-Y", strtotime(htmlentities($data['caveat_date'], ENT_QUOTES))) . '%0A';
                    }

                    //---MAIL MESSAGE--------//
                    $diary_no_mail = '<b>DIARY NUMBER : </b>' . htmlentities($data['diary_no'], ENT_QUOTES) . '<br>';
                    $case_title_mail = '<br><b>CAUSE TITLE : </b>' . htmlentities($data['pet_name'], ENT_QUOTES) . '<b> Vs. </b>' . htmlentities($data['res_name'], ENT_QUOTES) . '<br>';
                    ...
                    //---WHATSAPP MESSAGE--------//
                    $diary_no_whats = '*DIARY NO* : ' . htmlentities($data['diary_no'], ENT_QUOTES) . '%0A';
                    $case_title_whats = '*CAUSE TITLE* : ' . htmlentities($data['pet_name'], ENT_QUOTES) . ' *Vs*. ' . htmlentities($data['res_name'], ENT_QUOTES) . '%0A';
                    ...
                    ?>
                    <tr>
                        <td><?php echo echo_data($i++); ?></td>
                        <td><a href="<?=CASE_STATUS_API?>?d_no=<?php echo echo_data(substr_replace($data['diary_no'], "", -4)); ?>&d_yr=<?php echo echo_data(substr($data['diary_no'], -4)); ?>" target="_blank"><?php echo echo_data(substr_replace($data['diary_no'], "", -4)); ?> / <?php echo echo_data(substr($data['diary_no'], -4)); ?> <br><?php echo echo_data($data['reg_no_display']); ?> </a>  </td>
                        <td><a href="<?=CASE_STATUS_API?>?d_no=<?php echo echo_data(substr_replace($data['diary_no'], "", -4)); ?>&d_yr=<?php echo echo_data(substr($data['diary_no'], -4)); ?>" target="_blank">
                                <?php
                                if ($data['pno'] == 2) {
                                    $causeTitle = $data['pet_name'] . " AND ANR";
                                } elseif ($case_details['pno'] > 2) {
                                    $causeTitle = $data['pet_name'] . " AND ORS";
                                } else {
                                    $causeTitle = $data['pet_name'];
                                }
                                if ($data['rno'] == 2) {
                                    $causeTitle = $causeTitle . ' <b> Vs. </b> ' . $data['res_name'] . " AND ANR";
                                } elseif ($data['rno'] > 2) {
                                    $causeTitle = $causeTitle . '  <b> Vs. </b>  ' . $data['res_name'] . " AND ORS";
                                } else {
                                    $causeTitle = $causeTitle . '  <b> Vs. </b>  ' . $data['res_name'];
                                }
                                echo $causeTitle;
                                ?>
                            </a>
                        ...
                        <td>
                            <a href="<?=CASE_STATUS_API?>?d_no=<?php echo echo_data(substr_replace($$data['diary_no'], "", -4)); ?>&d_yr=<?php echo echo_data(substr($data['diary_no'], -4)); ?>" target="_blank"></a>
                        </td>
                        <td>
                            <a href="<?=CASE_STATUS_API?>?d_no=<?php echo echo_data(substr_replace($data['diary_no'], "", -4)); ?>&d_yr=<?php echo echo_data(substr($data['diary_no'], -4)); ?>" target="_blank">&nbsp;&nbsp;
                                <a data-toggle="modal" href="#add_citation_modal1" id="add_citation" class="add_citation_value" data-id="<?php echo echo_data(substr_replace($data['diary_no'], "", -4)); ?> / <?php echo echo_data(substr($data['diary_no'], -4)); ?> " title="Add Citation"><i class="fa fa-book" style="font-weight: bold; font-size: 20px;"></i></a>&nbsp;&nbsp;
                                <a data-toggle="modal" href="#add_notes_modal" id="add_notes" class="add_notes_value" data-id="<?php echo echo_data(substr_replace($data['diary_no'], "", -4)); ?> / <?php echo echo_data(substr($data['diary_no'], -4)); ?>" title="Add Notes"><i class="fa fa-file-text-o " style="font-weight: bold; font-size: 20px;"></i></a> &nbsp;&nbsp;
                                <a data-toggle="modal" onclick="view_contact_details('<?php echo url_encryption($data['diary_no']); ?>')" id="add_contact" class="add_contact_value" data-id="<?php echo $data['diary_no']; ?>" title="Add Contact"><i class="fa fa-phone" style="font-weight: bold; font-size: 20px;"></i></a>&nbsp;&nbsp;
                                ...
                                echo '<input class="col-md-1" type="hidden"  id="case_details_' . $i . '" value="' . $mail_message . '" >
                                      <input class="col-md-1" type="hidden"  id="subject_case_details_' . $i . '" value="' . $data['diary_no'] . '" >&nbsp;&nbsp;
