...
<?php if ($esigned_docs_details[0]->is_data_valid == 't') { ?>
    ...
    <?php } elseif ($esigned_docs_details[0]->signed_type == 3) { ?>
        <div class=" text-center">
            ...
            <p> <b> <a href="<?= base_url('affirmation/viewSignedCertificate/' . htmlentities(url_encryption(trim($esigned_docs_details[0]->id)), ENT_QUOTES)) ?>" target="_blank">
                        <i class="fa fa-file-pdf-o danger"  style="color:red"></i> View  eVerification
                    </a> 
                </b>
            </p>
            <?php if ($this->uri->segment(2) != 'view') { ?>
                <p> <button type="button" id="new_case_adv"  data-docid="<?php echo_data(url_encryption(trim($esigned_docs_details[0]->id).'$$reset'));?>" class="btn btn-danger reset_affirmation">Reset</button></p>
            <?php } ?>
        </div>
    <?php } else { ?>
        <div class="row" style="text-align: center" >
