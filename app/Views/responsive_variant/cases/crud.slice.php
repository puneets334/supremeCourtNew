...
?>
<div class="form-response" id="msg" style="background-color: green; color: #fff;    text-align: center;">
</div>
...
<div uk-filter="target: .js-filter">
    ...
    <div class="row">
        ...
        <?php //var_dump($arguingCounsel);  exit;
        if(isset($srAdvocate) && !empty($srAdvocate) && isset($diaryDetails->diary_no) && !empty($diaryDetails->diary_no)){
        echo '
            <div class="col-md-3">                
                
                    <select class="form-control filter_select_dropdown col-sm-8"  id="srAdvocate" name="srAdvocate" >
                        ...
                        if(isset($srAdvocate) && !empty($srAdvocate)){
                            foreach ($srAdvocate as $k=>$v) {
                                ...
                                if (isset($first_name) && !empty($first_name)
                                    && isset($middle_name) && !empty($middle_name)
            ...
            <select class="form-control filter_select_dropdown"  id="srAdvocate" name="srAdvocate" >
            ...
            <div class="col-md-3">
                <button type="button" style="margin-top: -6px;" id="addAdvocate" name="addAdvocate" class="btn btn-primary">Add Advocate</button>
            </div>
            <div class="row">
                <div class="panel panel-default" style="margin-top: 56px;">
                    <div class="panel-body">
            ...
            if(isset($arguingCounselList) && !empty($arguingCounselList)){
                $i=1;
              //  echo '<pre>'; print_r($arguingCounselList); exit;
  ...
  $(document).ready(function(){
    ...
    $(document).on('click','#addAdvocate',function(){
        ...
        if(advocateId){
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo base_url('newcase/Ajaxcalls/assignSrAdvocate'); ?>",
    ...
    $(document).on('click','.deleteSrAdvocateData',function(){
        ...
        if(confirm('Are you sure to '+engageMessage + user_type+' ?')){
            if(srAdvocateId){
                ...
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url('newcase/Ajaxcalls/deleteSrAdvocate'); ?>",
                    data: JSON.stringify(postData),
                    dataType:'json',
                    contentType:'application/json',
                    beforeSend: function () {
