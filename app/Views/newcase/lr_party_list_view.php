        ...
        <div class="col-md-12 col-sm-12 col-xs-12">             
            <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                ...
                <tbody>
                    <?php
                    $i = 1;
                    $pary_as = '';//echo '<pre>';print_r($lr_parties_list); echo '</pre>';
                    foreach ($lr_parties_list as $exp) { //echo '<pre>'; print_r($exp); exit;

                        $text = "2.4.1.3.5";// for test to ascending order
                        ...
                        <tr>
                            ...
                            <td><?php
                                foreach($part_id as $x => $x_value) {  if (!empty($x_value) && $x_value !=null){ echo $x_value.',';} }
                            ?></td>
                            <td><?php echo_data(strtoupper($e_type)); ?></td>

                            <?php if ($exp['party_type'] == 'I') { ?>
                                <td><?php echo_data(strtoupper($exp['lr_of_name'])); ?></td>
                                <td><?php if (!empty($party_type)){echo_data(strtoupper($party_type));echo '<br>';}
                                ...
                                <td><?php if (!empty($exp['party_age'])){ echo_data($exp['party_age'] . ' Yrs'); } if (!empty($dob)){ echo '(' . $dob . ' )';} ?></td>
                            <?php } elseif ($exp['party_type'] == 'D1') { ?>
                                <td><?php if (!empty($party_type)){echo_data(strtoupper($party_type));echo '<br>';}?> </td>
                            <?php }
                            ...
                            <td><?php echo_data(strtoupper($exp['lrs_remark'])); ?></td>
                            <?php
                            if($hidepencilbtn!='true'){ ?>
                                <td class="efiling_search"><a href="<?php echo base_url('newcase/lr_party/' . url_encryption($exp['id'])); ?>">Edit</a>&nbsp;&nbsp;&nbsp;<a href="<?php echo base_url('newcase/deleteParty/' . url_encryption($exp['id'])); ?>">Delete</a></td>
