...
if(!empty($_SESSION['efiling_details']['ref_m_efiled_type_id']) && $_SESSION['efiling_details']['ref_m_efiled_type_id'] == E_FILING_TYPE_CAVEAT)
    ...
    $this->load->view('newcase/new_case_breadcrumb');
?>
...
if ($this->uri->segment(2) == 'caseDetails') {
    ...
    $this->load->view('newcase/case_details_view');     
    
} elseif ($this->uri->segment(2) == 'petitioner') {
    
    $this->load->view('newcase/petitioner_view');
    
} elseif ($this->uri->segment(2) == 'respondent') { 
    
    $this->load->view('newcase/respondent_view');    
    
} elseif ($this->uri->segment(2) == 'extra_party') {
    
    $this->load->view('newcase/extra_party_view');     
    
} elseif ($this->uri->segment(2) == 'lr_party') {
    
    $this->load->view('newcase/lr_party_view');     
    
} elseif ($this->uri->segment(2) == 'actSections') {
   
    $this->load->view('newcase/act_sections_view');
    ...
    $this->load->view('newcase/subordinateCourt_view');
    ...
    if($this->session->efiling_details['is_govt_filing']==1)
    ...
    $this->load->view('newcase/new_case_preview');
}
?>
</div>
</div>
