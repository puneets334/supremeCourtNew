                    ...
                    foreach ($extra_parties_list as $exp) {
                        ...
                        <tr>
                            ...
                            <?php if ($is_party_type == 'I') { ?>
                                ...
                                <td><?php if (!empty($exp['party_name'])){ echo_data($exp['party_age'] . ' Yrs'); } if (!empty($dob)){ echo '(' . $dob . ' )';} ?></td>
                            <?php } elseif ($is_party_type == 'D1' || $is_party_type == 'D2') { ?>
                                <td> <?php if (!empty($party_type)){echo_data(strtoupper($party_type));echo '<br>';}if (!empty($exp['fetch_org_state_name'])){echo_data(strtoupper($exp['fetch_org_state_name'].', '));}
                                      echo_data($exp['org_post_not_in_list']=='t'?$exp['org_post_name'].', ':strtoupper($exp['authdesc'].', ')) . echo_data(strtoupper($exp['org_dept_not_in_list']=='t'?$exp['org_dept_name']:$exp['deptname'])); ?>
                                </td>
                                <?php if (!empty($party_type_hd) && $party_type_hd=='I'){ ?><td></td><?php }?>
