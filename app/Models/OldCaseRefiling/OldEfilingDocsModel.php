...
class OldEfilingDocsModel extends Model {
    ...
    public function gen_efiling_number()
    {
        ...
        $builder->where('entry_for_type', $session->get('estab_details')['efiling_for_type_id']);
        $builder->where('ref_m_establishment_id', $session->get('estab_details')['efiling_for_id']);
        ...
        if (count($row) > 0) {
            ...
            if ($year < date('Y')) {
                ...
                $update_data = [
                    ...
                    'doc_updated_by' => $session->get('login')['id'],
...
// defined in common model, ia_model also
        ...
        if ($query->getNumRows() >= 1) {
            ...
            $efiling_details = $query->getResult();
