...
class New_case_model extends Model {
    ...
    function gen_efiling_number() {
        ...
        $this->db->SELECT('case_efiling_no,case_efiling_year');
        ...
        if ($year < date('Y')) {
            $newYear = date('Y');
            $update_data = array('case_efiling_no' => 1,
                'case_efiling_year' => $newYear,
                'case_updated_by' => $this->session->userdata['login']['id'],
                'case_updated_on' => date('Y-m-d H:i:s'),
                'case_update_ip' => getClientIP());
            $this->db->WHERE('case_efiling_no', $p_efiling_num);
            $this->db->WHERE('case_efiling_year', $year);
            $this->db->WHERE('entry_for_type', $_SESSION['estab_details']['efiling_for_type_id']);
            $this->db->WHERE('ref_m_establishment_id', $_SESSION['estab_details']['efiling_for_id']);
            ...
            $efiling_num_info = array('case_efiling_no' => $gen_efiling_num,
                'case_updated_by' => $this->session->userdata['login']['id'],
                'case_updated_on' => date('Y-m-d H:i:s'),
                'case_update_ip' => getClientIP());
            $this->db->WHERE('case_efiling_no', $p_efiling_num);
    ...
    function add_efiling_nums($generated_efil_num, $curr_dt_time) {
        ...
        if ($this->db->insert_id()) {
            $session_data = array('registration_id' => $this->db->insert_id(), 'efiling_no' => $efiling_num);
    ...
    function add_update_case_parties($registration_id, $data, $breadcrumb_step, $party_id = null) {
        ...
        if ($party_id) {
            ...
            $this->db->UPDATE('efil.tbl_case_parties', $data);
            if ($this->db->affected_rows() > 0) {
                return TRUE;
            } else {
                return FALSE;
            ...
            $this->db->INSERT('efil.tbl_case_parties', $data);
    ...
    function update_party_lr_status($registration_id, $party_id, $curr_dt_time) {
        ...
        $data = array('have_legal_heir' => TRUE,
        ...
        $this->db->WHERE('have_legal_heir', FALSE);
        $this->db->UPDATE('efil.tbl_case_parties', $data);
        if ($this->db->affected_rows() > 0) {
            return TRUE;
        } else {
    ...
    function add_update_case_details($registration_id, $data, $breadcrumb_step, $case_details_id = null) {
        ...
        if ($case_details_id) {
            ...
            $this->update_breadcrumbs($registration_id, $breadcrumb_step);
            $this->db->INSERT('efil.tbl_case_details', $data);
    ...
    function add_update_adv_details($registration_id, $data) {
        ...
        /* if ($case_details_id) {
          ...
          return FALSE;
            ...
            $this->db->INSERT('efil.tbl_case_advocates', $data);
    ...
    function add_subordinate_court_info($registration_id, $data, $breadcrumb_step,$fir_data,$subordinate_court_details) {
        ...
        {
            $curr_dt_time = date('Y-m-d H:i:s');
            $data_deleted = array('is_deleted' => TRUE,
                'deleted_by' => $_SESSION['login']['id'],
                'deleted_on' => $curr_dt_time,
                'deleted_by_ip' => getClientIP());
            ...
            $this->db->WHERE('registration_id', $registration_id);
            $this->db->WHERE('is_deleted', FALSE);
            $this->db->UPDATE('efil.tbl_lower_court_details', $data_deleted);
        }
        else
            $this->update_breadcrumbs($registration_id, $breadcrumb_step);
            $this->db->INSERT('efil.tbl_lower_court_details', $data);
            $insert_id = $this->db->insert_id();
    
            if($fir_data!=null){
                $fir_data['ref_tbl_lower_court_details_id']=$insert_id;
                $this->db->INSERT('efil.tbl_fir_details', $fir_data);
            }
    ...
    function update_subordinate_court_info_newcase($id,$registration_id,$data) {
            ...
            $this->db->WHERE('registration_id', $registration_id);
            $this->db->WHERE('is_deleted', FALSE);
            $this->db->UPDATE('efil.tbl_lower_court_details', $data);
...
///////////////////////////////////////////////////////////////////////////////////
    ...
    function get_application_data($table_id, $login_id, $efiling_for_type_id) {
        ...
        if ($efiling_for_type_id == E_FILING_FOR_HIGHCOURT) {
            ...
            $this->db->JOIN('efil.tbl_efiling_nums en', 'en.registration_id = mvc.ref_m_efiling_nums_registration_id', 'left');
    ...
    function get_allocated_to_details($registration_id) {
        ...
        $this->db->SELECT("efil.tbl_efiling_nums.*,concat(users.first_name,' ',users.last_name) as admin_name,(CASE WHEN efil.tbl_efiling_nums.efiling_for_type_id =" . E_FILING_FOR_ESTABLISHMENT . " THEN (SELECT concat(estname,', ',dist_name,', ',state)  FROM m_tbl_establishments est
                ...
                as efiling_for_name");
        $this->db->FROM('efil.tbl_efiling_nums');
        $this->db->JOIN('efil.tbl_users as users', 'users.id = efil.tbl_efiling_nums.allocated_to', 'LEFT');
    ...
    function get_data($table, $filter_array, $select='*', $ordering='1') {
        $query = $this->db->WHERE($filter_array)->SELECT($select)->order_by($ordering)->GET($table);
        if ($query->num_rows() >= 1) {
            $result = $query->result_array();
            return $result;
        } else {
