...
class GetAiMiscDefectsCisStatusModel extends Model {
    ...
    function update_icmis_case_status($registration_id, $next_stage, $curr_dt, $case_details, $objections_insert, $objections_update, $documents_update,$efiling_type=null) {
        ...
        // }
        if ($next_stage) {
            // Assuming `update_next_stage` method exists and handles the logic
            $res = $this->update_next_stage($registration_id, $next_stage, $curr_dt);
        }
    ...
    function get_ai_misc_stage_and_registration_id($efiling_no){
        ...
        // $this->db->DISTINCT();
        // $this->db->SELECT(array('users.first_name as filed_by','users.userid','users.aor_code','en.created_by','cs.stage_id','en.registration_id', 'en.ref_m_efiled_type_id', 'en.efiling_no','cs.activated_on','en.efiling_for_id','efiling_type','mdia.diary_no','mdia.diary_year','concat(mdia.diary_no,mdia.diary_year)diary_no'));
        ...
        // $this->db->where('en.efiling_no', $efiling_no);
        // $this->db->WHERE_IN('cs.stage_id', [I_B_Approval_Pending_Admin_Stage,I_B_Defected_Stage,IA_E_Filed,Document_E_Filed]);
        // $query = $this->db->get();
    ...
    function get_ai_misc_stage_update_timestamp($registration_id, $stage_id){
        ...
        // $this->db->WHERE_IN('stage_id', $stage_id);
        // $this->db->WHERE("registration_id", $registration_id );
        // $this->db->WHERE('is_active', TRUE);
